...
#define loadingfname "data/tracks/track_001_.3DS"
//#define loadingfname "data/Park.3ds"

void DrawBboxOctree(COctree *pNode)
{
	if(!pNode) return;

	if(!g_Frustum.CubeInFrustum(pNode->GetCenter().x, pNode->GetCenter().y, 
								pNode->GetCenter().z, pNode->GetWidth() / 2) )
	{
		return;
	}
	if(pNode->IsSubDivided())
	{
		DrawBboxOctree(pNode->m_pOctreeNodes[TOP_LEFT_FRONT]);
		DrawBboxOctree(pNode->m_pOctreeNodes[TOP_LEFT_BACK]);
		DrawBboxOctree(pNode->m_pOctreeNodes[TOP_RIGHT_BACK]);
		DrawBboxOctree(pNode->m_pOctreeNodes[TOP_RIGHT_FRONT]);
		DrawBboxOctree(pNode->m_pOctreeNodes[BOTTOM_LEFT_FRONT]);
		DrawBboxOctree(pNode->m_pOctreeNodes[BOTTOM_LEFT_BACK]);
		DrawBboxOctree(pNode->m_pOctreeNodes[BOTTOM_RIGHT_BACK]);
		DrawBboxOctree(pNode->m_pOctreeNodes[BOTTOM_RIGHT_FRONT]);
	}
	else
	{
	    CVector3 _min;
        CVector3 _max;
        _min.x = pNode->GetCenter().x - pNode->GetWidth() / 2;
        _min.y = pNode->GetCenter().y - pNode->GetWidth() / 2;
        _min.z = pNode->GetCenter().z - pNode->GetWidth() / 2;
        _max.x = pNode->GetCenter().x + pNode->GetWidth() / 2;
        _max.y = pNode->GetCenter().y + pNode->GetWidth() / 2;
        _max.z = pNode->GetCenter().z + pNode->GetWidth() / 2;
        BBOX bbox(&_min,&_max);
        bbox.render();
	}
}
...
void example()
{
  ...
  CLoad3DS cload;  
  add_in_log("loading 3ds file %s\n",loadingfname);  
  cload.Import3DS(&g_3DModel, loadingfname);
  set_textures_g_3DModel(&g_3DModel, g_Textures, "data/tracks");
  //set_textures_g_3DModel(&g_3DModel, g_Texture, "data");
  debug_g_3DModel(&g_3DModel);
  g_Texture=g_Textures;
  
  COctree g_Octree;
  
  g_MaxTriangles = 50;
  g_Octree.GetSceneDimensions(&g_3DModel);
  int TotalTriangleCount = g_Octree.GetSceneTriangleCount(&g_3DModel);
  g_Octree.CreateNode(&g_3DModel, TotalTriangleCount, g_Octree.GetCenter(), g_Octree.GetWidth());
  g_Octree.SetDisplayListID( glGenLists(g_EndNodeCount) );
  g_Octree.CreateDisplayList(&g_Octree, &g_3DModel, g_Octree.GetDisplayListID());
...
/////////////////////////////////////   
    ...
    g_Frustum.CalculateFrustum();
    //glScalef(0.25f, 0.25, 0.25); 
    g_Octree.DrawOctree(&g_Octree, &g_3DModel);
    DrawBboxOctree(&g_Octree);
...
/*    
    ...
    //glScalef(1.5f, 1.5, 1.5);     
	//glRotatef(g_RotateX, 0.0, 1.0f, 0.0);
    //draw_t3DModel(&g_3DModel, g_ViewMode, g_Texture);                 
